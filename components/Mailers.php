 ...
 */
class Mailers extends \dektrium\user\Mailer {
    ...
    protected function sendMessage($to, $subject, $view, $params = [])
    {
        ...
        return $mailer->compose(['html' => $view, 'text' => 'text/' . $view], $params)
            ->setTo($to)
            ->setFrom(['hello@algn.me' => 'Algn'])
            ->setSubject($subject)
            ->send();
    }
    
      public function sendRecoveryMessageOtp(User $user, Token $token)
    {
         return $this->sendMessageOtp(
            $user->email,
            $this->getRecoverySubject(),
            'recovery2',
            ['user' => $user, 'token' => $token]
...
//*********************OTP TO EMAIL
	  public function sendRecoveryMessageOtpToEmail(User $user, Token $token)
    {
         return $this->sendMessageOtpToEmail(
            $user->email,
            $this->getRecoverySubjectToEmail(),
            'sendOtpToEmail',
            ['user' => $user, 'token' => $token]
        );
    }
   ...
   protected function sendMessageOtpToEmail($to, $subject, $view, $params = [])
    {
        $mailer = $this->mailerComponent === null ? Yii::$app->mailer : Yii::$app->get($this->mailerComponent);
        $mailer->viewPath = $this->viewPath;
        $mailer->getView()->theme = Yii::$app->view->theme;
    ...
    public function getRecoverySubjectToEmail()
    {
        if ($this->recoverySubject == null) {
           // $this->setRecoverySubject(Yii::t('user', 'Complete password reset on {0}', Yii::$app->name));
			$this->setRecoverySubject(Yii::t('user', 'ALGN account conformation code ', Yii::$app->name));
        }
...
//**********************************
     ...
     * @return bool
     */
    protected function sendMessageOtp($to, $subject, $view, $params = [])
    {
        $mailer = $this->mailerComponent === null ? Yii::$app->mailer : Yii::$app->get($this->mailerComponent);
        $mailer->viewPath = $this->viewPath;
        $mailer->getView()->theme = Yii::$app->view->theme;
