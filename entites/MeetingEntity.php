...
class MeetingEntity {
    ...
    public function auto_genrate_reoccur() {
        ...
        $this->model_meeting->setAttributes([
          ...
          'status' => $this->model_meeting_original->status != '' ? $this->model_meeting_original->status : 1,
        ]);
    ...
    public function save() {
       ...
       // print_r($this->form_model_meeting->attributes);die;
        if ($this->validate()) {
	...
	public function exitFromMeeting($form_accept_decline) {
			$meeting_user = MeetingUser::find()->where(['meeting_id' => $form_accept_decline->meeting_id, 'user_id' =>$form_accept_decline->meeting_user_model->user_id])->one();
			$meeting_user->availability = $form_accept_decline->availability;
        if ($meeting_user->availability == MeetingUser::MEETING_AVAILABILITY_STATUS_NOT_REQUIRED) {
            $meeting_user->decline_reason = $form_accept_decline->decline_reason;
			$meeting_user->status = MeetingUser::MEETING_AVAILABILITY_STATUS_NOT_REQUIRED;
			if ($meeting_user->save()) {
				$active_meeting_user_all = MeetingUser::find()->where(['meeting_id' => $form_accept_decline->meeting_id, 'created_by' =>$this->model_meeting->created_by,'status'=>1])->all();
				$meeting_user_active=[];
				if($active_meeting_user_all){
					foreach($active_meeting_user_all as $active_muser):
					$meeting_user_active[]=$active_muser->user_id;
					endforeach;
					$output = implode(',', $meeting_user_active);
					$this->model_meeting->meeting_user_id=$output;
					$this->model_meeting->update();
				}
				$this->genrateNotificationExitfrommeeting("exitfrommeeting", $meeting_user->user_id);
                return true;
            }
        }
        return false;
	...
	public function getDetailAll() {
		$this->model_meeting->status_type=1;
		$this->model_meeting->update();
		
        $meeting = \app\helpers\Utility::object_to_array($this->model_meeting);
		$meeting['users'] = \app\models\MeetingUser::find()->where(["meeting_id" => $this->model_meeting->id,"created_by"=>$this->model_meeting->created_by])
		->andWhere(['IN', 'user_id',explode(',',$this->model_meeting->meeting_user_id )])->all();
        ...
        $actions_temp = \app\models\ActionPoint::findAll(["meeting_id" => $this->model_meeting->id]);
         ...
         $meeting['orders'] = \app\models\MeetingOrder::findAll(["meeting_id" => $this->model_meeting->id]);
           ...
           $meeting['notification']= \app\models\Notification::find()->select(['notification.id','notification.detail_id','notification.user_id','notification.user_name','notification.mail_status','notification.status'])
    ...
    public function getDetail() {
        $meeting = \app\helpers\Utility::object_to_array($this->model_meeting);
		if($this->model_meeting->created_by==\Yii::$app->user->identity->id){
			if($this->model_meeting->status !=0){
				$this->model_meeting->status_type=0;
				$this->model_meeting->update();
			}else{
				$meetingstatus = \app\models\MeetingUser::findOne(["meeting_id" => $this->model_meeting->id,"status"=>1,"user_id"=>\Yii::$app->user->identity->id,"created_by"=>$this->model_meeting->created_by]);
				if($meetingstatus)
				$this->model_meeting->status_type=0;
				$this->model_meeting->update();
				}
		}
		if($this->model_meeting->meeting_group_id !=0){
			$selected_groups_ids = explode(",", $this->model_meeting->meeting_group_id);
			 foreach($selected_groups_ids as $row => $grp_id): 
							...
							\app\models\MeetingUser::updateAll(['as_user_or_group' => 1], ['AND','meeting_id ="' . $this->model_meeting->id . '" and status=1',['IN', 'user_id', $groups_member_id]]);
			endforeach;
		}
...
//          $meeting['user'] = \app\modules\api\v1\models\User::find()->where(['IN', 'id',explode(',',$this->model_meeting->meeting_user_id )])->all();
	...
	public function genrateNotificationExitfrommeeting($type = "", $activity_user_id = 0){
        \app\models\Notification::updateAll(['notification_status' => 0], "detail_id =" . $this->model_meeting->id);
        $this->model_user_created_by = \app\modules\api\v1\models\User::findOne($this->model_meeting->created_by);
        $this->model_user_updated_by = \app\modules\api\v1\models\User::findOne($this->model_meeting->updated_by);
        $this->model_user_responsible = \app\modules\api\v1\models\User::findOne($this->model_meeting->responsible_user_id);
        
       if ($type == "exitfrommeeting") {
				foreach ($this->model_meeting->meetingusers as $meeting_user_model) {
                    if($meeting_user_model->user_id==\Yii::$app->user->identity->id){
						//$notfication_service = new NotificationService($meeting_user_model->user_id, Notification::NOTIFICATION_TYPE_MEETING, Notification::NOTIFICATION_SUB_TYPE_MEETING_EXIT_OTHER);
						...
						$notfication_service = new NotificationService($this->model_meeting->created_by, Notification::NOTIFICATION_TYPE_MEETING, Notification::NOTIFICATION_SUB_TYPE_MEETING_EXIT_OTHER);
						$notfication_service->sendMeetingNotification($this,$activity_user_id);
						//$notfication_service = new NotificationService($this->model_meeting->created_by, Notification::NOTIFICATION_TYPE_MEETING, Notification::NOTIFICATION_SUB_TYPE_MEETING_EXIT_SELF);
						//$notfication_service->sendMeetingNotification($this,$activity_user_id);
            }
    }
