   ...
   public function actionListall() {
    ...
    public function actionList() {
	 ...
	 public function actionUsername(){
	  ...
	  public function actionReadstatus(){
           if (isset($this->data_json['action_id']) && $this->data_json['action_id'] != '' && isset($this->data_json['user_id']) && $this->data_json['user_id'] != '') {
				$action_id=$this->data_json['action_id'];
				$user_id=$this->data_json['user_id'];
				$read_action_user=\app\models\ActionUser::find()->where(["action_id" => $action_id,'user_id'=>$user_id,'status'=>1])->one();
				if($read_action_user){
						\app\models\ActionUser::updateAll(['read_status' => 1,'read_date_time'=>date('Y-m-d:h-i-d')], "action_id =" . $action_id." and user_id =".$user_id);
						
                    $this->response['action_user_detail']=\app\models\ActionUser::find()->where(["action_id" => $action_id,'user_id'=>$user_id,'status'=>1])->one();
                    return $this->response;
               }
               else{
                   throw new \yii\web\BadRequestHttpException("Bad Request, Action and User id not found"); // HTTP COde 400
               }
            }
           else{
                   throw new \yii\web\BadRequestHttpException("Bad Request, user id"); // HTTP COde 400
           }
      }
                ...
                if($action_model != null){
                    if($action_model->status == '0'){
                      // throw new \yii\web\ForbiddenHttpException("Sorry, This action is already Complete / Differ.");
						$name=\app\models\UserModel::find()->where(['id'=>$action_model->created_by])->one()->name;
						if($name){
							$this->response['message'] = " Sorry, This action has been deleted by ".$name;
						}else{
							$this->response['message'] = " Sorry, This action has been deleted by creator";
						}
    ...
    private function getLastMeetingUpdateTime($last_update_time = 000000) {
      ...
      public function actionSearchaction() {
        ...
        $dataProvider = $searchModel->searchaction(Yii::$app->request->queryParams);
	...
	public function actionSearchactionpoint() {
        $searchModel = new \app\models\ActionPointSearch();
		 if (isset($this->data_json['action'])){ 
			$searchModel->action = $this->data_json['action'];
		 }else{
			 $searchModel->action = '';
		 }
