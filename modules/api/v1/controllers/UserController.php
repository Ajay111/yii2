...
public function actionSignupusingemailotp(){
				   ...
				   // if($expire){
						...
						if ($userModel){
							if (isset($this->data_json['password'])){
								...
								if($this->data_json['username']){
									$username=$this->data_json['username'];
								}else{
									$username=$this->data_json['email'];;
								}
								$userModel->password=$this->data_json['password'];
								$userModel->name=$name;
								$userModel->email=$this->data_json['email'];
								$userModel->phone_no=$phone_no;
								$userModel->username=$phone_no;
    ...
    public function actionLogin() {
          if (isset($this->data_json['phone_no']) && $this->data_json['phone_no'] != ''){
               ...
               // $substr_phone_no = substr($phone_no, 0);
				 $substr_phone_no = $this->data_json['phone_no'];
    ...
    public function actionUpdatepassword() {
		...
		function sendOtpMessage($to,$otp,$type){
        			if($type==1)
						$message = "One time password for algn registration is $otp.";
						if($type==2)
						$message = "One time password for ALGN login is $otp.";
					$username="pboalgn";
					$password ="Algn@2019";
					$number=$to;
					$sender="INALGN";
    ...
    function sendOtpMessageCopy($to,$otp,$type){
...
//            }
 ...
 public function actionWbslist() {
    ...
    private function processLoginConsumer() {
          if (isset($this->data_json['phone_no'])) {    
            $model = \Yii::createObject(LoginForm::className());
...
//    }
    ...
    public function actionUserlist() {
	 $group=array();
        ...
        $connection = Yii::$app->getDb();
        $command = $connection->createCommand("SELECT u.id,u.name,u.created_at,u.email,u.phone_no,u.username,u.profile_image,u.status,u.blocked_at,u.confirmed_at FROM user JOIN user_registration as ur on user.id=ur.user_id JOIN user as u on u.id= ur.root_user_id WHERE user.org_id='".\Yii::$app->user->identity->org_id."' AND user.status=1 and u.id <>0 and user.id= $id");
        $resultSets=[];         
		  foreach($command->queryAll() as $key => $data):
                    $resultSets[$key]['id']=$data['id'];
