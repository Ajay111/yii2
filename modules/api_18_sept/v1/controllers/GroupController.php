...
class GroupController extends \yii\web\Controller
{
 ...
 $group=array();
       ...
       // $command = $connection->createCommand("SELECT `user_group`.* FROM `user_group` INNER JOIN `user` ON FIND_IN_SET(user.id,user_group.users) > '0' AND user.id='".\Yii::$app->user->identity->id."' WHERE user_group.status=1 GROUP BY `user_group`.`id` ORDER BY `user_group`.`id` DESC");
        $resultSets = [];
        if(count($command->queryAll()) > 0)
	    	{
		foreach($command->queryAll() as $key => $data):
