...
//public $response = [];
	...
	$username=$user->username;
        ...
        if ($user->create()) {
             ...
             $count=count($this->data_json['users']);
             if($count >0){
                  for($i=0; $i < $count;$i++){
                    $phone_no=$this->data_json['users'][$i]['mobile_no'];
                    ...
                    $user_registration= new \app\models\UserRegistration();
                        ...
                        $user_registration->name=$this->data_json['users'][$i]['name'];
                        $user_registration->phone_no=$this->data_json['users'][$i]['mobile_no'];
	...
	public function actionWebregistration() {
        $this->webprocessReg();
        return $this->response;
    }
	private function webprocessReg(){
		 if (!isset($this->data_json['name'])) {
			    throw new \yii\web\BadRequestHttpException("Name is Not Empty ! "); // HTTP COde 400
		 }
		 if (!isset($this->data_json['email'])) {
			    throw new \yii\web\BadRequestHttpException("Email is Not Empty ! "); // HTTP COde 400
		 }
		 if (!isset($this->data_json['phone_no'])) {
			    throw new \yii\web\BadRequestHttpException("Mobile No is Not Empty ! "); // HTTP COde 400
		 }
         if (isset($this->data_json['username']) and isset($this->data_json['password'])) {
             $user = \Yii::createObject([
                    'class' => \app\models\UserModel::className(),
        ...
        $user->role = \app\models\UserModel::ROLE_ORG_USER;
		$user->org_id = \app\models\UserModel::ORG_USER;
        $user->status = 1;
        $user->name=$this->data_json['name'];
        $user->email=$this->data_json['email'];
        $user->username=$this->data_json['phone_no'];
        $user->phone_no=$this->data_json['phone_no'];
        $user->password=$this->data_json['password'];
		$username=$user->username;
        $email=$user->email;
    ...
    public function actionContactsync(){
        ...
        if($user_model){
             ...
             if($count >0){
                  for($i=0; $i < $count;$i++){
                    $phone_no=$this->data_json['users'][$i]['mobile_no'];
                    $user_model =  \app\models\UserRegistration::find()->where(['user_id'=>$this->data_json['user_id'],'phone_no'=>$phone_no])->one();
                    if($user_model){
//                        $user_model->name=$this->data_json['users'][$i]['name'];
//                        $user_model->save();
                       }
                     else{
                  ...
                  $this->response['user'] = \app\models\UserRegistration::find()->where(['status' => 1,'user_id'=>$this->data_json['user_id']])->all();
                 return $this->response;
    ...
    public function actionUpdateprofileCopy() {
        ...
        if ($user->save()) {
          ...
          $this->trigger(self::EVENT_AFTER_UPDATE, $event);
          $this->response['user_list'] = $this->getUserprofile();
          return $this->response;
        }
    throw new NotFoundHttpException('The requested page does not exist');
    }
...
//            }
 ...
 public function actionWbslist() {
         ...
         if (isset($this->data_json['username']) and isset($this->data_json['password'])) {
           ...
           if ($model->login()) {
			   $last_login_time=date('Y-m-d H:i:s');
				\app\models\UserModel::updateAll(['last_login_time' => $last_login_time, 'login_status' => 1], "id =" . \Yii::$app->user->identity->id);
			     if (isset($this->data_json['web']) && $this->data_json['web']==1) {
					 $activity_user = \app\models\UserModel::findOne(\Yii::$app->user->identity->id);
					$member_app=AppDetail::find()->where(['user_id' => \Yii::$app->user->identity->id, 'status' => 1])->one();
					$this->response['status'] = "1";
					$this->response['user_id'] = \Yii::$app->user->identity->id;
					$this->response['user_name'] =$activity_user->username;
					$this->response['logged_in_user_name'] =$activity_user->name;
					$this->response['app_id'] = $member_app->id;
					$this->response['imei_no'] = $member_app->imei_no;
					$this->response['app_version'] = $member_app->app_version;
					$this->response['os_version'] = $member_app->os_version;
				    return $this->response['user_detail'] = $activity_user;
    ...
    private function processLoginIntoDb($confirm_overwrite = FALSE) {
		...
		$member_app_model->firebase_token_web = $this->data_json['firebase_token'];
    ...
    private function getUserprofile($last_update_time = 000000) {
  ...
  //  }
    public function actionUserlist($last_update_time = 000000) {
        ...
        $resultSets=[];         
		  foreach($command->queryAll() as $key => $data):
                    ...
                    $resultSets[$key]['last_login_time']=$data['last_login_time'];
                    $resultSets[$key]['last_logout_time']=$data['last_logout_time'];
					$resultSets[$key]['login_status']=$data['login_status'];
    ...
    $wbs=array();
        $org_id=\Yii::$app->user->identity->org_id;
                 $owner=\Yii::$app->user->identity->id;
         $wbs_list= \app\models\Wbs::find()->joinWith(['wbsusers'])->where(['=', 'wbs_user.created_by', \Yii::$app->user->identity->id])
 ...
 private function getLastGroupUpdateTime($last_update_time = 000000) {
    ...
    private function getMeetinglist($last_update_time = 000000) {
			...
			return \app\models\Meeting::find()
                ->joinWith(['meetingusers'])
                ->where(['or', 'meeting_user.user_id='.\Yii::$app->user->identity->id,'meeting.responsible_user_id=' . \Yii::$app->user->identity->id])
				->andWhere(['between', 'meeting.start_datetime', $start_date, $end_date])
				->andWhere(['=', 'meeting.org_id',\Yii::$app->user->identity->org_id])
				->orderBy('meeting.id DESC')
                ->all();
     }
       ...
       // return  \app\models\ActionPoint::find()
        ...
        //        ->all();
			...
			return  \app\models\ActionPoint::find()
                ...
                ->where(['or', 'action_user.user_id='.\Yii::$app->user->identity->id,'action_point.action_assigned_by=' . \Yii::$app->user->identity->id])
                //->andWhere(['=', 'meeting_id', '0'])
				->andWhere(['between', 'action_point.deadline', $start_date, $end_date])
				->andWhere(['=', 'action_point.org_id',\Yii::$app->user->identity->org_id])
                ->orderBy('action_point.id DESC')
                ->all();
        }
	...
	public function actionLogout() {
		...
		\app\models\UserModel::updateAll(['last_logout_time' => $last_logout_time, 'login_status' => 0], "id =" . \Yii::$app->user->identity->id);
		$activity_user = \app\models\UserModel::findOne(\Yii::$app->user->identity->id);
		$this->response['status'] ="1";
