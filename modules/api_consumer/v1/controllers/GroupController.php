...
public function actionExitgroup(){
 ...
 public function actionExitgroupBackup(){
		 if (isset($this->data_json['group_id']) && $this->data_json['group_id'] != '') {
			$group_id=$this->data_json['group_id'];
			$group_model = \app\models\Group::findOne(['id' => $this->data_json['group_id'],'status'=>1]);
            if($group_model == null) {
				...
				$group_user_model =  \app\modules\api\v1\models\GroupUser::find()->where(['group_id'=>$group_id,'user_id'=>\Yii::$app->user->identity->id,'status'=>1])->one();
				if($group_user_model==null){
					$this->response['status'] = "0";
					$this->response['message'] = "You have already exit from group"; 
					
				}else{
				\app\modules\api\v1\models\GroupUser::updateAll(['status' => 0], 'user_id ="' . \Yii::$app->user->identity->id . '" and group_id='. $group_id);
				$group_user = \app\modules\api\v1\models\GroupUser::findAll(["group_id" => $group_id,'status' => 1]);
				$users='';
				foreach ($group_user as $grp):
				$users.=$grp->user_id.',';
				endforeach;
				$users=rtrim($users,',');
					\app\models\Group::updateAll(['users' => $users], "id =" . $group_model->id);
					$this->response['status'] = "1";
					$this->response['message'] = "You are successfully exit from group"; 
					$group_by_model = \app\models\Group::find()->where(['id' => $this->data_json['group_id'],'status'=>1])->all();
					$resultSets = [];
					foreach($group_by_model as $key => $group):
					$resultSets[$key]['id'] = $group->id;
					$resultSets[$key]['group_name'] = $group->group_name;
					$resultSets[$key]['users'] = $group->users;
					$resultSets[$key]['status'] = $group->status;
					$resultSets[$key]['group_user'] = \app\modules\api\v1\models\GroupUser::find()->where(['group_id'=>$group_id])->all();
					endforeach;
					$this->response['data']=$resultSets;
					//$this->response['group']=\app\modules\api\v1\models\GroupUser::find()->where(['group_id'=>$group_id])->all();
				}
              }
			
			 }else{
				$this->response['status'] = "0";
                $this->response['message'] = "Group is not empty"; 
			 }
		return $this->response;
        }
   ...
   public function actionCreate11() {
    ...
    public function actionListall()
		...
		$command = $connection->createCommand("SELECT `user_group`.* FROM `user_group` INNER JOIN `user` ON FIND_IN_SET(user.id,user_group.users) OR FIND_IN_SET(user.id,user_group.created_by) where user.id='".\Yii::$app->user->identity->id."' and user_group.status=1 GROUP BY `user_group`.`id` ORDER BY `user_group`.`group_name` ASC");
